...
def load_data(model_args):
    ...
    cold_user_ratio = getattr(args, 'cold_user_ratio', 0)
    if cold_user_ratio > 0:
        print(f"[Cold-Start] Enabling user cold-start experiment with ratio = {cold_user_ratio}")
        all_users = np.unique(train_cf[:, 0])
        cold_users, warm_users = train_test_split(all_users, test_size=1 - cold_user_ratio, random_state=42)

        # 移除冷启动用户的训练交互
        cold_train_cf = np.array([pair for pair in train_cf if pair[0] not in cold_users])
        # 仅保留冷启动用户的测试交互
        cold_test_cf = np.array([pair for pair in test_cf if pair[0] in cold_users])

        # 构建新的 user_dict
        cold_user_dict = {
            'train_user_set': {u: items for u, items in train_user_set.items() if u not in cold_users},
            'test_user_set': {u: items for u, items in test_user_set.items() if u in cold_users}
        }

        return cold_train_cf, cold_test_cf, cold_user_dict, n_params, graph, [adj_mat_list, norm_mat_list,
                                                                              mean_mat_list]
