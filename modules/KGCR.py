...
from .AttnAggregator import AttnAggregator
...
class KGCR(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, hp_dict=None):
        ...
        if args_config.dataset == 'lastfm2k':
            self.cl_coef = 0.1
            self.tau = 0.6
            self.cl_drop = 0.7
        elif args_config.dataset == 'ml100k':
            self.cl_coef = 0.01
            self.tau = 1.0
            self.cl_drop = 0.9
        elif args_config.dataset == 'book-crossing':
            self.cl_coef = 0.1
            self.tau = 0.2
            self.cl_drop = 0.3
        ...
        self.gcn = AttnAggregator(channel=self.emb_size,
    ...
    def print_shapes(self):
        ...
        self.logger.info("context_hops: %d", self.context_hops)  # layer_num
