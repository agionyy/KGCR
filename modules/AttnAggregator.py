...
class AttnAggregator(nn.Module):
    ...
    def norm_attn_computer(self, entity_emb, edge_index, edge_type=None, print=False, return_logits=False):
        ...
        edge_attn_logits = edge_attn.mean(-1).detach()  # f(h,r,t)
        # softmax by head_node
        edge_attn_score = scatter_softmax(edge_attn_logits, head)  # w(h,r,t)
        ...
        edge_attn_score = edge_attn_score * norm  # r(h,r,t)
        # print attn score
        if print:
            self.logger.info("edge_attn_score std: {}".format(edge_attn_score.std()))
        if return_logits:
            return edge_attn_score, edge_attn_logits  # return r(h,r,t), f(h,r,t)
